...
- **Drizzle** for schema and migrations (decided 8 Sep 2026). Drizzle owns
  the **single forward-only migration chain**: schema in TypeScript, SQL
  migrations generated by `drizzle-kit` and committed. The Supabase CLI is
  used **only** to run a local Postgres/Auth instance in dev and CI. Never
  run `supabase db push` or `supabase migration`; doing so forks the history.
...
| 9   | Admin and operations          | Audit log UI, score overrides, season export, backup monitoring                          |
...
- **Zod at every API boundary.** Every request body, search param, form
  ...
  before it is used (`src/lib/env.ts`, `src/lib/schemas/`). Parse, don't
  validate.
- **Generated, not hand-written, types for API responses.** Response types are
  inferred from the Zod schema or from the Drizzle schema (`src/db/types.ts`,
  regenerated by `npm run db:generate-types`, checked in CI). Never duplicate
  a shape by hand.
- **Forward-only, versioned migrations.** One migration chain, owned by
  Drizzle (`drizzle-kit generate`, committed SQL). Never edit a migration
  that has been applied anywhere; write a new one. Never use the Supabase
  CLI for migrations.
- **Database access model.** Request code queries only through `withUser`
  (`src/db/client.ts`), which opens a transaction, `SET LOCAL ROLE wows_app`
  and sets `app.user_id`; RLS policies in
  `drizzle/migrations/0001_roles_triggers_rls.sql` do the scoping. The app
  uses the **transaction-mode pooler**: `SET LOCAL` is transaction-scoped and
  Supavisor pins one backend per transaction, so nothing leaks; session-level
  `SET` is forbidden; prepared statements are off. `src/db/system.ts` (and
  `src/lib/supabase/admin.ts`) bypass RLS and may only be imported by
  `scripts/` and tests, never `src/app` (guarded by a test).
- **Never `supabase db push`.** Drizzle owns migrations. The Supabase CLI
  exists here only to run the local instance.
- **Roles.** `user_roles` is additive. `applicant`, `core`, `faculty`, `alum`
  are global (`season_id` NULL); `member` and `lead` are per season. `core`
  is deliberately global because core must be able to create the first
  season, so there is nothing for a per-season core role to attach to. Who
  leads a vertical is derived from the season's `lead` role plus that user's
  membership vertical; there is no `verticals.lead_user_id`.
- **Forecast editability.** A forecast may be revised until `closes_at`; the
  trigger locks it after, against server time, and increments
...
- **A failed write surfaces to the user** (H34). Never catch, log, and render
  as if it succeeded.
- **Structured logging with request IDs.** `src/proxy.ts` assigns
  `x-request-id`; server code logs through `requestLogger()` in
  `src/lib/log.ts`, one JSON object per line. Never `console.log` a bare
  string in server code. Sentry (`@sentry/nextjs`) is inert without a DSN.
...
- H1. Every mutating endpoint checks role server-side. Hiding a button is not authorization.
  - Tested by: `e2e/login.spec.ts` (signed-in member POSTs to `/api/admin/roles`, gets 403; signed-out gets 401) and `src/lib/auth.test.ts` (role matching). Every mutating endpoint must call `requireRole` (`src/lib/auth.ts`).
- H2. Row-level security or equivalent server-side scoping on every table containing member data. A member cannot read another member's open positions, unresolved forecasts, or draft research by manipulating an ID. There must be an explicit test.
  - Tested by: `src/db/rls.db.test.ts` — as member A, through `withUser` with `requireRole` deliberately bypassed, B's open position, thesis, unresolved forecast and draft research note return no rows; guest sees nothing; A cannot write in B's name; faculty cannot write. `src/lib/repo-invariants.test.ts` keeps the RLS-bypassing system path out of `src/app`.
...
- H4. No member's live season-portfolio positions are visible to other members while open; theses become visible after the season settles.
  - Tested by: `src/db/rls.db.test.ts` ("H4": B's position and thesis become readable by A only once the season is settled).
...
- H8. `orders`, `fills`, and `audit_log` are append-only. `orders` unique on `(run_id, idempotency_key)`. `audit_log` is never deleted.
  - Tested by: `src/db/invariants.db.test.ts` (orders, fills, audit_log append-only; orders unique on `(run_id, idempotency_key)`).
- H9. Theses are immutable after submission; edits create a new revision row.
  - Tested by: `src/db/invariants.db.test.ts` (theses UPDATE rejected; revision rows unique per position). Submission flow: Phase 5.
- H10. Forecasts are immutable after `closes_at`; unique on `(question_id, user_id)`.
  - Tested by: `src/db/invariants.db.test.ts` (one per `(question_id, user_id)`; writes rejected after `closes_at` by server time; `revised_count` increments).
...
- H20. Forecast submissions lock at the deadline; late submissions rejected server-side against server time. All deadlines enforced against server time.
  - Tested by: `src/db/invariants.db.test.ts` (forecast deadline enforced against `now()` in the database). Other deadlines: with their features.
...
- H31. Audit log: every admin action that changes member-visible state is recorded with actor, timestamp, before, after; viewable in-app.
  - Tested by: `src/db/invariants.db.test.ts` (audit_log append-only) and `src/db/rls.db.test.ts` (staff-only read). Storage only; the in-app viewer and full admin-action coverage: Phase 9.
...
- H34. A failed write surfaces to the user. Never catch, log, and render as if it succeeded.
  - Tested by: dashboard only — `src/app/(member)/dashboard/error.tsx` surfaces a failed load with a retry; `src/app/auth/confirm/route.ts` signs the user out rather than proceeding when registration fails. First scored write: Phase 4.
- H35. Automated daily database backup with a documented, _tested_ restore procedure.
  - Tested by: `scripts/backup-drill.sh` runs in CI on every push (dump, wipe, restore, compare counts). Production backup: `.github/workflows/backup.yml` (dormant until enabled, README > Backups). Monthly manual drill documented in README.
